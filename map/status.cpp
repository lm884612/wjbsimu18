...
static pec_ushort status_calc_str(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_agi(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_vit(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_int(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_dex(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_luk(struct block_list *,struct status_change *,int);
static pec_ushort status_calc_pow(struct block_list *, struct status_change *, int);
static pec_ushort status_calc_sta(struct block_list *, struct status_change *, int);
static pec_ushort status_calc_wis(struct block_list *, struct status_change *, int);
static pec_ushort status_calc_spl(struct block_list *, struct status_change *, int);
static pec_ushort status_calc_con(struct block_list *, struct status_change *, int);
static pec_ushort status_calc_crt(struct block_list *, struct status_change *, int);
...
static pec_short status_calc_mdef2(struct block_list *,struct status_change *,int);
...
static pec_short status_calc_patk(struct block_list *, struct status_change *, int);
static pec_short status_calc_smatk(struct block_list *, struct status_change *, int);
static pec_short status_calc_res(struct block_list *, struct status_change *, int);
static pec_short status_calc_mres(struct block_list *, struct status_change *, int);
static pec_short status_calc_hplus(struct block_list *, struct status_change *, int);
static pec_short status_calc_crate(struct block_list *, struct status_change *, int);
...
#ifdef RENEWAL
static pec_ushort status_calc_ematk(struct block_list *,struct status_change *,int);
#endif
...
bool status_calc_weight(struct map_session_data *sd, enum e_status_calc_weight_opt flag)
{
	...
	if (flag&CALCWT_MAXBONUS) {
		...
		if (sc->data[SC_KNOWLEDGE])
			add_max_weight += sd->max_weight * sc->data[SC_KNOWLEDGE]->val1 / 10;
		if ((skill = pc_checkskill(sd, ALL_INCCARRY)) > 0)
			add_max_weight += 2000 * skill;
		if (pc_ismadogear(sd))
 ...
 */
static pec_ushort status_calc_str(struct block_list *bl, struct status_change *sc, int str)
{
	if(!sc || !sc->count)
		return cap_value(str,0,PEC_USHRT_MAX);
 ...
 */
static pec_ushort status_calc_agi(struct block_list *bl, struct status_change *sc, int agi)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_vit(struct block_list *bl, struct status_change *sc, int vit)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_int(struct block_list *bl, struct status_change *sc, int int_)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_dex(struct block_list *bl, struct status_change *sc, int dex)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_luk(struct block_list *bl, struct status_change *sc, int luk)
{
	if(!sc || !sc->count)
...
*/
static pec_ushort status_calc_pow(struct block_list *bl, struct status_change *sc, int pow)
{
	if (!sc || !sc->count)
...
*/
static pec_ushort status_calc_sta(struct block_list *bl, struct status_change *sc, int sta)
{
	if (!sc || !sc->count)
...
*/
static pec_ushort status_calc_wis(struct block_list *bl, struct status_change *sc, int wis)
{
	if (!sc || !sc->count)
...
*/
static pec_ushort status_calc_spl(struct block_list *bl, struct status_change *sc, int spl)
{
	if (!sc || !sc->count)
...
*/
static pec_ushort status_calc_con(struct block_list *bl, struct status_change *sc, int con)
{
	if (!sc || !sc->count)
...
*/
static pec_ushort status_calc_crt(struct block_list *bl, struct status_change *sc, int crt)
{
	if (!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_batk(struct block_list *bl, struct status_change *sc, int batk)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_watk(struct block_list *bl, struct status_change *sc, int watk)
{
	if(!sc || !sc->count)
 ...
 */
static pec_ushort status_calc_ematk(struct block_list *bl, struct status_change *sc, int matk)
{
	if (!sc || !sc->count)
 ...
 * @return modified matk with cap_value(matk,0,USHRT_MAX)
 */
static pec_ushort status_calc_matk(struct block_list *bl, struct status_change *sc, int matk)
{
	if(!sc || !sc->count)
		return cap_value(matk,0,PEC_USHRT_MAX);
#ifndef RENEWAL
 ...
 */
static pec_short status_calc_critical(struct block_list *bl, struct status_change *sc, int critical)
{
	if(!sc || !sc->count)
 ...
 */
static pec_short status_calc_hit(struct block_list *bl, struct status_change *sc, int hit)
{
	if(!sc || !sc->count)
 ...
 */
static pec_short status_calc_flee(struct block_list *bl, struct status_change *sc, int flee)
{
	if( bl->type == BL_PC ) {
 ...
 */
static pec_short status_calc_flee2(struct block_list *bl, struct status_change *sc, int flee2)
{
	if(!sc || !sc->count)
 ...
 */
static pec_defType status_calc_def(struct block_list *bl, struct status_change *sc, int def)
{
	if(!sc || !sc->count)
 ...
 */
static pec_short status_calc_def2(struct block_list *bl, struct status_change *sc, int def2)
{
	if(!sc || !sc->count)
#ifdef RENEWAL
 ...
 */
static pec_defType status_calc_mdef(struct block_list *bl, struct status_change *sc, int mdef)
{
	if(!sc || !sc->count)
 ...
 */
static pec_short status_calc_mdef2(struct block_list *bl, struct status_change *sc, int mdef2)
{
	if(!sc || !sc->count)
#ifdef RENEWAL
...
*/
static pec_short status_calc_patk(struct block_list *bl, struct status_change *sc, int patk)
{
	if (!sc || !sc->count)
...
*/
static pec_short status_calc_smatk(struct block_list *bl, struct status_change *sc, int smatk)
{
	if (!sc || !sc->count)
...
*/
static pec_short status_calc_res(struct block_list *bl, struct status_change *sc, int res)
{
	if (!sc || !sc->count)
...
*/
static pec_short status_calc_mres(struct block_list *bl, struct status_change *sc, int mres)
{
	if (!sc || !sc->count)
...
*/
static pec_short status_calc_hplus(struct block_list *bl, struct status_change *sc, int hplus)
{
	if (!sc || !sc->count)
...
*/
static pec_short status_calc_crate(struct block_list *bl, struct status_change *sc, int crate)
{
	if (!sc || !sc->count)
